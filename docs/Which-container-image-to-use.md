`vaultwarden` 提供单个 Docker 镜像 ([`vaultwarden/server`](https://hub.docker.com/r/vaultwarden/server))，统一支持 SQLite、MySQL 和 PostgreSQL 数据库后端，从版本开始1.17.0。在该版本之前，每个数据库后端都有单独的镜像（请参阅 [历史镜像](#historical-images)）。
